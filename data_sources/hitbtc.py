...
async def get_prices(symbols: List[str]) -> List[str]:
    ...
    symbol_pairs = [SYMBOL_MAP[sym] for sym in symbols]

    r = requests.get(f"{URL}/api/3/public/ticker", params={"symbols": symbol_pairs})
    r.raise_for_status()

    usdt_price = await get_usdt_usd_rate()

    price_map = {sym: Decimal(data["last"]) for sym, data in r.json().items()}
    return [
        "{:f}".format((price_map[sym_pair] * usdt_price).normalize())
        for sym_pair in symbol_pairs
    ]
